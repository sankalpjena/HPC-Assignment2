...
int main(int argc, char *argv[]) {
  ...
  for (tx = 0; tx < n_threads; ++tx) {
    ...
    *args = tx;
    ...
    if (ret = pthread_create(&compute_threads[tx], NULL, compute_main,
                             (void *)args)) {
  ...
  for (size_t ix = offset; ix < n_lines; ix += n_threads) {
    ...
    */
    ...
    for (size_t jx = 0; jx < n_lines; ++jx) {
      ...
      // Default value of attr set to 99 as that doesnt correspond to any
      // root/color
      for (conv = 0, attr = 99;; ++conv) {

        // Abso
        if (creal(xk) > 1e10 || cimag(xk) > 1e10 || creal(xk) < -1e10 ||
            cimag(xk) < -1e10) {
          attr = 11; // A Default color (maroon) which is not associated to any
                     // actual root is assigned
          break;
        }

        double abs_real_xk = creal(xk) * creal(xk);
        double abs_imag_xk = cimag(xk) * cimag(xk);
        double radiussq = abs_real_xk + abs_imag_xk;
        // Absoulte value < lower bound
        if (radiussq < LOWER_BOUND) {
          attr = 10; // A Default color (maroon) which is not associated to any
                     // actual root is assigned
          break;
        }
        if ( radiussq < 1.002001 && radiussq > (1.-0.001)*(1.-0.001) )
        ...
        case 3:
          // STATEMENTS FOR DEGREE 3
          x_k1 = (0.666666666666667) * (xk) +
                 (0.333333333333333) * (1 / (xk * xk));
          break;

        case 4:
          // STATEMENTS FOR DEGREE 4
          x_k1 = (0.750000000000000) * (xk) +
                 (0.250000000000000) * (1 / (xk * xk * xk));
          break;

        case 5:
          // STATEMENTS FOR DEGREE 5
          x_k1 = (0.800000000000000) * (xk) +
                 (0.200000000000000) * (1 / (xk * xk * xk * xk));
          break;

        case 6:
          // STATEMENTS FOR DEGREE 6
          x_k1 = (0.833333333333333) * (xk) +
                 (0.166666666666667) * (1 / (xk * xk * xk * xk * xk));
          break;

        case 7:
          // STATEMENTS FOR DEGREE 7
          x_k1 = (0.857142857142857) * (xk) +
                 (0.142857142857142) * (1 / (xk * xk * xk * xk * xk * xk));
          break;

        case 8:
          // STATEMENTS FOR DEGREE 8
          x_k1 = (0.875000000000000) * (xk) +
                 (0.125000000000000) * (1 / (xk * xk * xk * xk * xk * xk * xk));
          break;

        case 9:
          // STATEMENTS FOR DEGREE 9
          x_k1 = (0.888888888888889) * (xk) +
                 (0.111111111111111) / (xk * xk * xk * xk * xk * xk * xk * xk);
          break;
        ...
        xk = x_k1; // Updating the value of xk with the newly computed value for
                   // the next iteraion
      }

      convergence[conv_x] = conv <= 51 ? conv : 51;
...
void *write_main(void *args) {
  ...
  char gray[52][13] = {
      "000 000 000 ", "005 005 005 ", "010 010 010 ", "015 015 015 ",
      "020 020 020 ", "025 025 025 ", "030 030 030 ", "035 035 035 ",
      "040 040 040 ", "045 045 045 ", "050 050 050 ", "055 055 055 ",
      "060 060 060 ", "065 065 065 ", "070 070 070 ", "075 075 075 ",
      "080 080 080 ", "085 085 085 ", "090 090 090 ", "095 095 095 ",
      "100 100 100 ", "105 105 105 ", "110 110 110 ", "115 115 115 ",
      "120 120 120 ", "125 125 125 ", "130 130 130 ", "135 135 135 ",
      "140 140 140 ", "145 145 145 ", "150 150 150 ", "155 155 155 ",
      "160 160 160 ", "165 165 165 ", "170 170 170 ", "175 175 175 ",
      "180 180 180 ", "185 185 185 ", "190 190 190 ", "195 195 195 ",
      "200 200 200 ", "205 205 205 ", "210 210 210 ", "215 215 215 ",
      "220 220 220 ", "225 225 225 ", "230 230 230 ", "235 235 235 ",
      "240 240 240 ", "245 245 245 ", "250 250 250 ", "255 255 255 "};
  ...
  for (size_t ix = 0; ix < n_lines;) {
    ...
    for (; ix < n_lines && item_done_loc[ix] != 0; ++ix) {
      int *attractor = attractors[ix];
      int *convergence = convergences[ix];

      // Write result
      // Creating ROW BUFFER
      char row_attr[n_lines * 12 + 1];
      char row_conv[n_lines * 12 + 1];

      for (size_t cx = 0; cx < n_lines; ++cx) {
        memcpy(row_attr + (cx * 12), colors[attractor[cx]], 12);
        memcpy(row_conv + (cx * 12), gray[convergence[cx]], 12);
        if (convergence[cx] > 51)
          fprintf(stderr, "HELP, accessing uninitialized memory for index %d\n",
                  convergence[cx]);
      }
